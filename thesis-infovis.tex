...
Information visualization seeks to use interactive graphics to present
and analyze abstract information more efficiently. These two goals of
presenting and analyzing abstract information in a visual form build
on the properties of human visual perception, which include the rapid
scanning, recognition, and recall of visual information, as well as
the automatic detection of patterns within it. In contrast to textual
representations of data, the processing of well-designed
visualizations requires much less cognitive effort, because it
leverages features of the human visual processing system. One of these
features is \emph{preattentive processing}, whereby certain visual
attributes can be processed very quickly and without conscious effort
...
assumptions, as demonstrated by Anscombe's famous visualization of
four completely different datasets having identical summary statistics
(mean and standard deviation), called Anscombe's Quartet
\parencite{AnscombesQuartet}.
% KA  need to see the data in a table too, to understand this!
An observer trying to understand these datasets purely from their
summary statistics would mistakenly deem them to be identical. Their
inequality only becomes obvious after carefully examining and
comparing the individual entries in the datasets themselves. This is a
tedious and error-prone task when unaided by visual representations
like Figure~\ref{fig:AnscombesQuartet}. Even though Anscombe's Quartet
is very likely the most famous example to demonstrate this
characteristic, it is certainly not the only example, as has been
shown by \textcite{GenDataIdenticalStatisticsDissimilarGraphics}.
...
Anscombe's Quartet consists of four distinct datasets which
share identical summary statistics. The difference between
these datasets is only apparent by carefully examining the
textual data, but very obvious when plotting it graphically.
...
visualization having three main subfields, as defined by
...
\end{enumerate}
The often-used term \enquote{data visualization} (DazaVis) is defined
as the combination of information visualization and geographic
visualization.
...
multidimensional data. Even though the majority of the attention in
the field of information visualization has been put on the
presentational aspect of visualizations, much research has also been
conducted on their interactive aspects. Many taxonomies have been
formulated with the goal of defining the design space of interactions
to support analytic reasoning, but they vary greatly depending on the
concepts they are focusing on. Some taxonomies have been defined on
the concept of low-level interaction techniques
...
% HERE  Read Yi paper and check table
...
\imgcredit{Adapted from \textcite{RoleOfInteractionInInformationVisualization}}
}
...
\caption[Chart of Planetary Movements from the Tenth Century]{
  ...
  \imgcredit{Image extracted from \textcite{BriefHistoryOfDataVis}. Original appearance in \textcite{CommentariiInSomniumScipionis}.}
}
...
\caption[Chart of Changes in Sunspots from 1626]{
  ...
  \imgcredit{Image extracted from \textcite{BriefHistoryOfDataVis}. Original appearance in \textcite{RosaUrsina}.}
}
...
\caption[Chart of Longitudinal Distance Determinations Between Toledo and Rome From 1644]{
  ...
  \imgcredit{Image extracted from \textcite{BriefHistoryOfDataVis}. Original appearance in \textcite{VisualExplanations}.}
}
...
\caption[Dot Map Plotting Cholera Deaths in London From 1855]{
  ...
  \imgcredit{Image extracted from \textcite{IVISCourseNotes}. Original appearance in \textcite{ModeOfCommunicationOfCholera}.}
}
...
\caption[Screenshot of High-D]{
  ...
  \imgcredit{Screenshot of \textcite{HighD} taken by the author of this work.} \TODO{How to cite this? Copyright?}
}
\label{fig:HighD}
\end{figure}
...
The visual aspects of a visualization are described in a grammar similar to the one defined by \textcite{GrammarOfGraphics}.
