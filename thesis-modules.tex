...
\lstinputlisting[%
  ...
  caption={[Basic Bar Chart Window Example]%
    ...
    The Bar Chart Window is configured with the bound data object that is initialized via the \code{chartBarWindowData} function.
    After configuration, the Chart Window is rendered by the \code{chartBarWindowRender} function.
    Since no special responsive behavior is desired in this example, the default resize and category filter behavior is attached to the Chart Window via the \code{chartBarWindowAutoResize} and \code{chartBarWindowAutoFilterCategories} functions. 
  },
...
\begin{figure}[tp]
\centering
\includegraphics[keepaspectratio,width=\linewidth,height=\fullh]{diagrams/bar-chart-window.pdf}
\caption[Basic Bar Chart Window Example]{
