...
configuration of Cartesian Charts via the \code{xAxis}, \code{yAxis},
\code{title}, and \code{subtitle} properties. Transposing Axes is a
useful pattern to improve the responsiveness of visualizations and can
be configured using the \code{flipped} boolean property of a Cartesian
Chart's data object. If the \code{flipped} property is set to
\code{false}, the \code{xAxis} object is used to configure the Bottom
Axis and the \code{yAxis} object is used to configure the Left Axis.
...
\elname{<svg>} element to it. The drawing area is the container element
...
\elname{<rect>} background element filling the whole drawing area is
...
layout. By default, a grid is created which defines the \code{header},
...
Apart from Tooltips, the Tooltip package also contains utility
...
provided to automatically set \code{pointerover}, \code{pointermove},
and \code{pointerout} event listeners on Series to automatically
...
\elname{<rect>} elements representing the bars in a Bar Chart's drawing
...
collection of \elname{<rect>} elements meant for display in the drawing
