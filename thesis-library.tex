...
\subsection{Extend Rather Than Wrap D3}
...
naming convention can be seen in the \code{d3-scale}
\parencite{D3Scale} and \code{d3-axis} \parencite{D3Axis} modules, in
...
\end{figure}
% KA These are not in alphabetical or listing order??
...
% KA styling of module names
The \code{core} module contains the core functionality of the library
...
and various utility functions. The \code{legend} module contains a
Legend Component, which renders a Legend consisting of a title and
configurable labeled symbols. The \code{tooltip} module contains
...
creation and rendering functions. The \code{bars} and \code{points}
...
\code{<script>} tag embedded in the body of the HTML document. In
...
Rollup creates source maps for the \code{dist/respvis.[m]js} and
...
installed when installing a local package. The \code{package.json}
...
documents like any other JavaScript resource via a \code{<script>}
...
a HTML document using a \code{<script>} element. However, it is
necessary to mark them as modules via the \code{type="module"}
attribute on the loading \code{<script>} element, so that browsers can
...
respectively. The \code{cleanDist} task is exported under a different
...
is executed as the first step of the \code{build} task. The public
