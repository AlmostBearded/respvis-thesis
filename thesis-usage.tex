...
\code{src/examples} directory of the RespVis library contains examples
...
\item Attach a \code{<div>} element to the element into which the
  Chart Window should be rendered.

\item Bind a fully-initialized data object to the \code{<div>} element
  ...
  partial object via one of the \code{chartWindowXData} functions.

\item Render the Chart Window using the appropriate
  \code{chartWindowXRender} function.

\item Attach a \code{resize} event listener to the \code{<div>}
...
The following list contains all the responsive patterns that can be
...
\item \liintro{Rotate tick labels}: One of the most effective ways to
  ...
  rotation in the CSS \code{transform} property on them and modifying
  their CSS \code{text-anchor} property accordingly.
...
\item \liintro{Remove ticks}: If neither rotation nor shortening of
  ...
  \code{display} property. A D3 Axis' \code{ticks} function allows
  ...
  \code{tickValues} function of a D3 Axis allows for much more control
  than the \code{ticks} function because it is used to specify the
...
\item \liintro{Simplify title/subtitle}: Since Axes do not just
  ...
  specifying shorter textual representations via the \code{title} and
  \code{subtitle} properties on Axes' data objects.
...
\item \liintro{Relocate title/subtitle}: The titles and subtitles of
  ...
  CSS \code{grid-template} property.

\item \liintro{Remove title/subtitle}: The titles and subtitles of
  Axes can be hidden via the CSS \code{display} property.
\end{itemize}
...
\item \liintro{Relocate Legend}: A Legend's position in its containing
  ...
  layout via the CSS \code{grid-template} property, or by positioning
  the Legend at predefined \code{\"top\"}, \code{\"right\"},
  \code{\"bottom\"}, and \code{\"left\"} positions via the
  \code{data-legend-position} attribute.
...
\item \liintro{Simplify title}: A Legend's title can be shortened by
  ...
  \code{title} property on a Chart's data object.

\item \liintro{Remove title}: If the title can not be further
  shortened or if it does not convey too much important information,
  it can be hidden via the CSS \code{display} property.
...
\item \liintro{Relocate labeled symbols}: Changing the layout of
  ...
  \code{flex-direction} property on their container element.
...
The following list shows the implementation of different responsive
...
\item \liintro{Reduce bar padding}: Reducing the paddings between bars
...
\item \liintro{Remove bar labels}: An alternative to prevent bar
  ...
  \code{opacity} or \code{display} properties.

\item \liintro{Transpose Chart}: By default, Bar Charts are rendered
  ...
  transposed by setting the \code{flipped} property on their data
  objects.
...
\item \liintro{Remove bars}: Sometimes it might be required to reduce
  ...
  declaring them as inactive via the \code{activeCategories} and
  \code{activeSubcategories} properties on a Bar Chart's data object.
  Using these properties is easier than manually adapting the
  \code{categories}, \code{values}, \code{categoryScale}, and
  \code{valueScale} properties on the data object. Furthermore,
  categories and subcategories that have been hidden via the
  \code{activeCategories} and \code{activeSubcategories} properties
  can also be unhidden by users via a Chart's Toolbar if they are
  interested in the additional data.

\item \liintro{Apply Axis patterns}: Since Bar Charts usually contain
...
\item \liintro{Apply Legend patterns}: For Multi-Series Bar Charts
...
\subref{fig:BarChartPatterns60rem} At a viewport size of 60rem, the
...
rotated.  \subref{fig:BarChartPatterns40rem} At a viewport size of
...
\subref{fig:BarChartPatterns30rem} At a viewport size of 30rem, the
...
\item \liintro{Simplify marker labels}: The labels of markers might
  ...
  \code{labels} property on a Line Chart's data object.

\item \liintro{Remove marker labels}: Sometimes shortening labels may not be
...
\item \liintro{Transpose Chart}: Even though Line Charts are less
  ...
  Charts can be transposed by setting the \code{flipped} property on
  their data objects.

\item \liintro{Apply Axis patterns}: As with all Charts that use Axes
  ...
  the CSS \code{display} property to turn Line Charts into Sparklines,
  but this should be considered carefully because information about
  the scale of data is lost when hiding Axes.

\item \liintro{Apply Legend patterns}: In Multi-Series Line Charts
...
\subref{fig:LineChartPatterns60rem} At a viewport size of 60rem, all
markers are shown and tick labels are not shortened.
\subref{fig:LineChartPatterns40rem} At a viewport size of 40rem, tick
labels are shortened.  \subref{fig:LineChartPatterns30rem} At a
...
\item \liintro{Rescale points}: When a large number of points are
  ...
  of points can be controlled via the \code{radiuses} property on the
...
\item \liintro{Simplify labels}: When labels are shown in a Point
  ...
  be configured via the \code{labels} property on the data objects of
  Point Charts.

\item \liintro{Remove labels}: Often, Point Charts are used for the
  ...
  might be better to hide them via the CSS \code{opacity} or
  \code{display} properties.

\item \liintro{Zoom draw area}: Zooming allows users to view data at
  ...
  are the \code{xScale} and \code{yScale} properties on the data
...
\item \liintro{Apply Axis patterns}: As with all Cartesian charts that
...
\item \liintro{Apply Legend patterns}: In the case of Point Charts
...
Figure~\ref{fig:PointChartPatterns}.  Depending on the screen width,
...
implementation have been removed for clarity reasons.
},
...
\subref{fig:PointChartPatterns60rem} At a viewport size of 60rem, all
ticks are shown, tick labels are not shortened, and points are
rendered at their full size.  \subref{fig:PointChartPatterns40rem} At
...
\subref{fig:PointChartPatterns30remZoomed} At a viewport size of
