...
\begin{enumerate}
% KA TODO: indicate which line numbers in Listing 7.1 !!
...
\item Attach a new \code{<div>} element to the root \code{<div>}
  element (the one with \code{id=\"chart\"}) [Line 7].
...
\item \liintro{Remove markers}: If a Chart contains a large number of
  ...
  is to hide them via the CSS \code{opacity} or \code{display}
  properties.
...
\item \liintro{Remove marker labels}: Sometimes, shortening marker
  ...
  them via the CSS \code{opacity} or \code{display} properties.
...
\item \liintro{Transpose Chart}: Even though Line Charts are less
  ...
  ones. Line Charts can be transposed by setting the \code{flipped}
...
\item \liintro{Apply Axis patterns}: As with all charts which use axes
  ...
  removed completely via the CSS \code{display} property, turning the
...
\subref{fig:LineChartPatterns60rem} At a width of 60rem, all
markers are shown and tick labels are not shortened.
\subref{fig:LineChartPatterns40rem} At a width of 40rem, tick
...
\item \liintro{Remove labels}: Often, Point Charts are used to
  ...
  better to hide them via the CSS \code{opacity} or \code{display}
  properties.
...
\item \liintro{Zoom draw area}: Zooming allows users to view data at
  ...
  which are the \code{xScale} and \code{yScale} properties of the data
...
\item \liintro{Apply Axis patterns}: As with all Cartesian charts
...
% KA TODO
% If there is size and color coding in a scatterplot,
% is this one Legend or two?
...
\subref{fig:PointChartPatterns60rem} At a width of 60rem, all
...
\subref{fig:PointChartPatterns30remZoomed} At a width of
