...
In addition to visuals being easier to assimilate by humans, a purely textual and statistical view on data can also lead to erroneous assumptions as demonstrated by \textcite{AnscombesQuartet} in the infamous visualization of four completely different datasets which have identical summary statistics, called Anscombe's Quartet.
...
Even though Anscombe's Quartet is very likely the most famous example to demonstrate this characteristic, it is certainly not the only set of datasets which possesses it, as has been shown by \textcite{GenDataIdenticalStatisticsDissimilarGraphics}.
...
\caption[Anscombe's Quartet]{
  ...
  \imgcredit{Image extracted from \textcite{IVISCourseNotes}. Used with kind permission by Keith Andrews.}
}
\label{fig:AnscombesQuartet}
\end{figure}

This thesis adheres to the separation of the field defined by \textcite{IVISCourseNotes}. 
...
Even though the majority of the attention in the field of information visualization has been put on the presentational aspect of visualizations, much research has also been conducted on their interactive aspects.
...
\textcite{RoleOfInteractionInInformationVisualization} aims to provide a view in between the purely system-centric and purely user-centric extremes by defining a taxonomy based on user intent.
...
\caption[Categories of Interaction Based on User Intent]{
  ...
  \imgcredit{Table adapted from \textcite{RoleOfInteractionInInformationVisualization}}
}
...
Other noteworthy early visualizations include the first occurrence of the principle \textcite{VisualDisplayOfQuantitativeInformation} later coined "small multiples" in the 1626 chart demonstrating sunspot changes seen in Figure~\ref{fig:SunspotChanges} by \textcite{RosaUrsina}, and the 1644 chart displaying longitudinal distance determinations between Toledo and Rome seen in Figure~\ref{fig:RomeToledoLongitude} by Michael Florent van Langren.
